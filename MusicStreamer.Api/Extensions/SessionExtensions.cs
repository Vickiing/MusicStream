...
public static class SessionExtensions
{
    ...
    public static void SetCurrentUser(this ISession session, SessionUserViewModel user)
    {
        session.SetString(SessionKey, JsonSerializer.Serialize(user));
    }

    public static SessionUserViewModel? GetCurrentUser(this ISession session)
    {
        var payload = session.GetString(SessionKey);
        return payload is null ? null : JsonSerializer.Deserialize<SessionUserViewModel>(payload);
    }
